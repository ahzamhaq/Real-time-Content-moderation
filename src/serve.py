...
def _assert_public_url(url: str) -> None:
    """Raise HTTPException if `url` resolves to a non-public address (SSRF guard).

    Blocks loopback, private, link-local, and other reserved ranges so
    /predict/url can't be used to reach internal services (e.g. localhost
    admin ports or the cloud metadata endpoint at 169.254.169.254).
    """
    import ipaddress
    import socket
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to resolve host: {e}")
    ...
    for info in infos:
        ...
        if (
            ...
            raise HTTPException(
                status_code=400,
                detail="URL resolves to a private/internal address, which is not allowed.",
            )
...
async def predict_url(request: URLRequest):
    ...
    # Fetch the page, manually following redirects so each hop is
    # re-validated (a public URL could otherwise redirect to an internal one).
    try:
        async with hx.AsyncClient(follow_redirects=False, timeout=15.0) as client:
